...
function readPedido(){
    ...
    .then(data =>{
        ...
        data.forEach((element,index) => {
            ...
            tabla += `<td>${element.formaPago}</td>`
            tabla += `<td>${element.fechaEnvPedido}</td>`
            tabla += `<td><button onclick='pedidoEstado(${element.id})' data-bs-toggle="modal" data-bs-target="#UpdateEstad">${element.estadoPedido}</button> </td>`
                tabla += `<td>${element.estado}</td>`
                tabla += `<td><button  data-bs-toggle="modal" data-bs-target="#UpdateModal">ver pedido</button></td>`
            tabla += `</tr>`     
        })
